...
public class ReviewableResolver {
    ...
    public Reviewable resolve(@NonNull UUID uuid, @NonNull ReviewableType type) {
        return (Reviewable) switch (type) {
            case TOUR -> touristRepository.findByUuid(uuid)
                    .orElseThrow(() -> {
                        log.error("Tour not found: {}", uuid);
                        return new ResourceNotFoundException("Tour", "uuid", uuid.toString());
                    });
